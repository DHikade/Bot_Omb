...
REG_PRIVILEGES = re.compile('!priv[\s]{1}.+[0-9]$')
REG_URL = re.compile('!url[\s]{1}.+[^\s]$')
REG_COMMAND = re.compile('!command[\s]{1}![a-z]+[\s]{1}[0-9]+[\s]{1}')
...
REG_LANG = re.compile('!lang[\s]{1}[a-z]{2}\r\n$')
...
REG_SETTING = re.compile('!setting[\s]{1}[\S]*[\s]{1}(on|off)$')
